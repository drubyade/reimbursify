...
export default function SubmitterLayout({
  ...
  const profileItems = [
    ...
    { label: "Dashboard", icon: BarChart2, href: "/trips/analytics", key: "analytics", colorClass: "bg-fuchsia-500" },
  ];

  let dynamicTitle = "Dashboard";
  if (pathname?.startsWith("/trips/analytics")) {
    dynamicTitle = "Dashboard";
  } else if (pathname?.startsWith("/trips")) {
    dynamicTitle = "My Trips";
  } else if (pathname?.startsWith("/groups")) {
    dynamicTitle = "My Groups";
        ...
        >
          ...
          <nav className="flex-1 overflow-y-auto py-4 space-y-1">
            <div className={`mb-2 flex items-center justify-center ${!sidebarOpen ? "px-0" : "px-6"}`}>
              {sidebarOpen ? (
                <p className="text-xs font-bold tracking-wider text-slate-400 uppercase w-full">Main</p>
              ) : (
            ...
            {navigationItems.map((item) => {
              const isActive = item.href === '/trips' 
                ? (pathname === '/trips' || (pathname?.startsWith('/trips/') && !pathname?.startsWith('/trips/analytics'))) ?? false
            ...
            <div className={`mt-8 mb-2 flex items-center justify-center ${!sidebarOpen ? "px-0" : "px-6"}`}>
              {sidebarOpen ? (
                <p className="text-xs font-bold tracking-wider text-slate-400 uppercase w-full">Account</p>
              ) : (
                <div className="w-8 h-px bg-slate-300 mx-auto"></div>
              )}
            </div>
          ...
          <div className="fixed inset-0 z-50 flex md:hidden">
            ...
            <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white shadow-xl">
              ...
              <div className="p-4 border-b border-gray-100 flex items-center gap-3 bg-[var(--bg-secondary)]">
                ...
                <div>
                  <p className="font-bold text-[var(--primary)] text-sm">{session?.user?.name || "User"}</p>
              ...
              <div className="flex-1 h-0 overflow-y-auto">
                <nav className="px-2 py-4 space-y-1">
                  <p className="px-3 text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Main</p>
                  {navigationItems.map((item) => {
                    const isActive = item.href === '/trips' 
                      ? (pathname === '/trips' || (pathname?.startsWith('/trips/') && !pathname?.startsWith('/trips/analytics'))) ?? false
                      : pathname?.startsWith(item.href) || false;
                  ...
                  <p className="px-3 text-xs font-bold text-gray-400 uppercase tracking-wider mt-6 mb-2">Account</p>
                  {profileItems.map((item) => (
                      <MobileNavLink
                        key={item.key}
                        href={item.href}
